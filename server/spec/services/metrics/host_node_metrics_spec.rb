...
describe Metrics::HostNodeMetrics do
  ...
  let(:stats) {
    HostNodeStat.create([
      ...
      { # 3) this record should be skipped (wrong grid)
        grid: other_grid,
      ...
      { # 5) This is included in second metric for grid
        grid: grid,
        host_node: node,
        cpu: {
          user: 0.25,
      ...
      { # 6) this record should be skipped
        grid: grid,
        host_node: node,
        cpu: {
          user: 0.2,
  ...
  describe '#fetch' do
    it 'returns aggregated data by node_id' do
      ...
      results = Metrics::HostNodeMetrics.fetch(node.id, from, to)
