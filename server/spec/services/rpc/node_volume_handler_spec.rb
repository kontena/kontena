...
describe Rpc::NodeVolumeHandler, celluloid: true do
  ...
  describe '#list' do
    it 'fails if host node does not exist' do
      ...
      expect{subject.list()}.to raise_error 'Node not found'
    end
    ...
    it 'does not return volumes from other node' do
      ...
      volume = grid.volumes.create!(driver: 'local', scope: 'instance', name: 'foo')
      volume.volume_instances.create!(host_node: other_node, name: 'svc.foo-1')
      list = subject.list()
  ...
  describe '#set_state' do
    ...
    it 'does not save volume instance if volume not found' do
      data['volume_id'] = 'foo'
      expect {
        expect{subject.set_state(data)}.to raise_error 'Could not find volume with id: foo'
