describe HostNodes::Common do
  ...
  context 'for a grid with three nodes' do
    ...
    let!(:node_a) { grid.host_nodes.create!(name: 'node-a', node_id: 'AA', connected: true) }
    let!(:node_b) { grid.host_nodes.create!(name: 'node-b', node_id: 'BB', connected: true) }
    let!(:node_c) { grid.host_nodes.create!(name: 'node-c', node_id: 'CC', connected: false) }
