...
class AccessToken
  ...
  field :deleted_at, type: Time, default: nil # for marking refresh tokens and codes as already used
  field :internal, type: Boolean, default: true # true for cli<->master tokens, false for master<->AP tokens
  field :code, type: String, default: nil # oauth authrorization code

  index({ user_id: 1 })
  index({ internal: 1 })
  index({ token: 1, internal: 1 }, { unique: true, })
  index({ refresh_token: 1 }, { sparse: true, unique: true })
  index({ expires_at: 1 }, { sparse: true })
  index({ deleted_at: 1 }, { sparse: true })
  index({ code: 1 }, { sparse: true, unique: true })

  # Plain text accessors to hashed tokens, available right after the 
  # token creation for internal tokens. After that you can not access
  ...
  class << self
    ...
    # Find encrypted token using a supplied access token
    #
    # @param [String] plain_text_access_token
    # @return [AccessToken]
    ...
    # Since we don't know the original saved tokens, we just delete the old one
    # and generate a duplicate with the same scope + user
    #
    # TODO consider hashing codes.
    def find_internal_by_code(code)
      coded_token = AccessToken.where(code: code, deleted_at: nil).
        find_and_modify({ '$set' => { deleted_at: Time.now.utc } })
  ...
  # Generates a random code and sets it to :code
  #
  # @param [Fixnum] number_of_bytes
  # @return [String] code
  ...
  # Time to token expiration in seconds
  #
  ...
  # Returns true if user has any of the requested scopes
