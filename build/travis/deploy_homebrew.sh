...
clone_repositories() {
  ...
  git clone https://github.com/Homebrew/homebrew-core.git homebrew-core
  cd homebrew-core
  git checkout master
  git remote add fork https://github.com/kontena/homebrew-core.git
  git push -f fork master
  cd ..

  # Clone the brew tap repository
  git clone https://github.com/kontena/homebrew-kontena.git homebrew-kontena
}
...
prerelease() {
  ...
  cat << EOB | ruby $CLI_VERSION $LONG_HASH
    ...
    formula.sub(bottle_block, "#{devel_block}\n#{bottle_block}")
    formula.sub("if build.head?", "if build.head? || build.devel?")
...
EOB
  ...
  cat << EOB | ruby > resources.rb
    require 'open-uri'
    resources = {}
    Dir["gems/**"].select { |p| File.directory?(p) && p !~ /extensions/ }.each do |path|
...
EOB
  ...
  cat << EOB | ruby $CLI_VERSION $LONG_HASH
    ...
    new_stable_block += File.read('tmp/resources.rb')
