...
describe Kontena::Cli::Stacks::UpgradeCommand do
  ...
  describe '#execute' do
    ...
    context 'with a stack including dependencies' do
      ...
      let(:response)     { JSON.parse(JSON.dump(expectation.merge(parent: nil, children: [{'name' => 'deptest-dep_1'}, {'name' => 'deptest-dep_2'}]))) }
      let(:response_1)   { JSON.parse(JSON.dump(expectation_1.merge(parent: { 'name' => 'deptest' }, children: [{'name' => 'deptest-dep_1-dep_1'}]))) }
      let(:response_1_1) { JSON.parse(JSON.dump(expectation_1_1.merge(parent: { 'name' => 'deptest-dep_1' }, children: []))) }
      let(:response_2)   { JSON.parse(JSON.dump(expectation_2.merge(parent: { 'name' => 'deptest' }, children: [], variables: {}, services: []))) }
