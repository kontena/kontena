...
class Kontena::Command < Clamp::Command
  ...
  def run(arguments)
    Kontena.logger.debug { "Running #{self} with #{arguments.inspect} -- callback matcher = '#{self.class.callback_matcher.nil? ? "nil" : self.class.callback_matcher.map(&:to_s).join(' ')}'" }
    @arguments = arguments
