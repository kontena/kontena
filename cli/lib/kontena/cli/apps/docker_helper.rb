module Kontena::Cli::Apps
  module DockerHelper
    ...
    def process_docker_images(services, force_build = false, no_cache = false)
      services.each do |name, service|
        if service['build'] && (!image_exist?(service['image']) || force_build)
          ...
          exit_with_error("'#{service['image']}' is not valid Docker image name") unless validate_image_name(service['image'])
          exit_with_error("'#{service['build']['context']}' does not have #{dockerfile}") unless dockerfile_exist?(service['build']['context'], dockerfile)
    ...
    def build_docker_image(service, no_cache = false)
      ...
      cmd << build_context
      ret = system(cmd.join(' '))
      exit_with_error("Failed to build image #{service['image'].colorize(:cyan)}") unless ret
      ret
    end
    ...
    def push_docker_image(image)
      ...
      exit_with_error("Failed to push image #{image.colorize(:cyan)}") unless ret
      ret
    end
    ...
    def run_pre_build_hook(hook)
      hook.each do |h|
        ret = system(h['cmd'])
        exit_with_error("Failed to run pre_build hook: #{h['name']}!") unless ret
      end
    end
  end
end
