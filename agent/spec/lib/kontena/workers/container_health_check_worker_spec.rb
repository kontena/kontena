...
describe Kontena::Workers::ContainerHealthCheckWorker do
  ...
  describe '#check_http_status' do
    ...
    it 'returns healthy status' do
      ...
      expect(Excon).to receive(:get).with('http://1.2.3.4:8080/health', {:connect_timeout=>10, :headers=>{"User-Agent"=>"Kontena/Agent"}}).and_return(response)
    ...
    it 'returns unhealthy status when connection timeouts' do
      expect(Excon).to receive(:get).with('http://1.2.3.4:8080/health', {:connect_timeout=>10, :headers=>{"User-Agent"=>"Kontena/Agent"}}).and_raise(Excon::Errors::Timeout)
      health_status = subject.check_http_status('1.2.3.4', 8080, '/health', 10)
      expect(health_status[:data]['status']).to eq('unhealthy')
    end

    it 'returns unhealthy status when connection fails with weird error' do
      expect(Excon).to receive(:get).with('http://1.2.3.4:8080/health', {:connect_timeout=>10, :headers=>{"User-Agent"=>"Kontena/Agent"}}).and_raise(Excon::Errors::Error)
      health_status = subject.check_http_status('1.2.3.4', 8080, '/health', 10)
      expect(health_status[:data]['status']).to eq('unhealthy')
    end
