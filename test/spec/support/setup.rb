module Setup
  ...
  def setup_master(version)
    ...
    k = Kommando.new "kontena vagrant master create --skip-auth-provider --name e2e --memory 512 --version #{version}", output: true
  ...
  def setup_grid(name, initial_size = 1, version = nil)
    ...
    3.times do |i|
      Kommando.run("kontena vagrant node create --instances 1 --memory 1024 --version #{version} #{name}-#{i + 1}", output: true)
    end
  end
