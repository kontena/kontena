...
The `kontena grid create --subnet=` option can be used to change the default `10.81.0.0/16` grid subnet used for overlay networking if this subnet overlaps with other internal networking uses on the grid's host nodes.
...
See the [Network Model](../core-concepts/networking.html#grid) documentation for further details.
...
The grid token is used to grant access to the Kontena master, as well as the shared secret used for the overlay networking encryption.

## Update options
These options can also be set using `kontena grid update` while the grid is running.

### Default Affinity

The `kontena grid create --default-affinity label!=special` option acts as if each Kontena service were configured with the given service affinity by default.
...
See the [Deployment Strategies](deploy.html#affinity) documentation for the affinity filter syntax.
...
See the [Statistics](stats.html#exporting-stats) documentation for further details.
